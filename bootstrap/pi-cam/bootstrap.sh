...
sudo cp ~/bootstrap/pi-cam/ustreamer@.service /etc/systemd/system/ustreamer@.service
...
sudo cp ~/bootstrap/pi-cam/ustreamer-proxy /etc/nginx/sites-available/ustreamer-proxy
